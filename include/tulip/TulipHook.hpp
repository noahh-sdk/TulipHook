...
namespace tulip::hook {
	TULIP_HOOK_DLL noahh::Result<HandlerHandle> createHandler(void* address, HandlerMetadata const& metadata) noexcept;

	TULIP_HOOK_DLL noahh::Result<void, std::string> removeHandler(HandlerHandle const& handler) noexcept;
	...
	TULIP_HOOK_DLL noahh::Result<void, std::string> writeMemory(void* destination, void const* source, size_t size) noexcept;

	TULIP_HOOK_DLL noahh::Result<void*, std::string> followJumps(void* address) noexcept;

	// wraps a cdecl function into given convention
	TULIP_HOOK_DLL noahh::Result<void*, std::string> createWrapper(void* address, WrapperMetadata const& metadata) noexcept;

	// wraps a function in given convention into cdecl
	TULIP_HOOK_DLL noahh::Result<void*, std::string> createReverseWrapper(void* address, WrapperMetadata const& metadata) noexcept;
	...
	TULIP_HOOK_DLL noahh::Result<void, std::string> disableRuntimeIntervening(void* commonHandlerSpace) noexcept;
