...
// virtual noahh::Result<RelocateReturn> relocatedBytes(int64_t original, int64_t relocated, size_t size);
...
noahh::Result<BaseGenerator::RelocateReturn> BaseGenerator::relocatedBytes(int64_t original, int64_t relocated, std::span<uint8_t const> originalBuffer, size_t targetSize) {
	return noahh::Ok(RelocateReturn{ std::vector<uint8_t>(), 0 });
}
